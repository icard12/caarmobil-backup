import { ChevronRight, ShoppingBag, Terminal, DollarSign, History, TrendingUp, TrendingDown, AlertTriangle, Check, X, Clock, Calendar } from 'lucide-react';
  ...
  return (
    <div className="space-y-4 lg:space-y-10 pb-10">
      ...
      <div className="flex items-center gap-2 lg:gap-4 overflow-x-auto hide-scrollbar pb-1 -mx-4 px-4 sm:mx-0 sm:px-0">
        <div className="flex items-center gap-2 bg-[var(--bg-panel)] p-1 rounded-xl border border-[var(--border-subtle)]">
          <button
            onClick={() => setSelectedDate(new Date().toISOString().split('T')[0])}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${selectedDate === new Date().toISOString().split('T')[0]
                ? 'bg-[#FF4700] text-white shadow-md'
                : 'text-[var(--text-muted)] hover:bg-[var(--bg-canvas)]'
              }`}
          >
            Hoje
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${selectedDate === new Date().setDate(new Date().getDate() - 1) ? '' : ''
          ...
          <div className="relative flex items-center">
            <Calendar className="w-3.5 h-3.5 absolute left-2.5 text-[var(--text-muted)] pointer-events-none" />
            <input
              type="date"
              value={selectedDate}
              onChange={(e) => setSelectedDate(e.target.value)}
              className="pl-8 pr-3 py-1.5 bg-transparent text-xs font-bold text-[var(--text-main)] outline-none cursor-pointer [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:w-full"
            />
          </div>
        </div>
      </div>
